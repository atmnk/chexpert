...
import { BaseChatModelCallOptions } from "@langchain/core/language_models/chat_models.js";
...
export class AI {
    ...
    constructor(page: Page, provider = (process.env.PROVIDER_NAME || "google"), modelName = (process.env.MODEL_NAME || "gemini-2.0-flash"), debug = false) {
        this.started = false
        this.debug = debug
        this.page = page;
        switch (provider) {
            case "google":
                ...
                this.model = model1.bind({
                    tools: [new NavigateTool(), new ClickTool(), new FillTextTool(), new CheckIfVisible()],
                });
                break
            case "anthropic":
            ...
            default:
                throw Error("Invalid provider name please set PROVIDER_NAME as either google or anthropic")
        }
    }
    setPage(page: Page) {
        this.page = page
    ...
    private async tce(page: Page, prompt: string, pollingInterval: number = 1000, iterations: number = 20, attempt: number = 0, scrollIfRequired: boolean = false) {
        if (!this.started) {
            ...
            await this.model.invoke([
                "system",
                "You are driver agent that drives automation specified in plain english, You have access to tools to accomplish steps provided subsequently during each interaction tools will execute code on playwright page and provide aria snapshot with of all elements"
            ])
            this.started = true
            // if(this.debug){
        ...
        // }
        // var state: DomState = await this.domService.getClickableElements(this.highlightElements, -1, 0);
